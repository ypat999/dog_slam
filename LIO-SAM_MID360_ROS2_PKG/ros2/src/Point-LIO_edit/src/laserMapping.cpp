...
{
    flg_exit = true;
    RCLCPP_WARN(nh_ptr->get_logger(), "catch sig %d", sig);
...
void publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr & pubOdomAftMapped)
{
    ...
    static tf2_ros::TransformBroadcaster br(nh_ptr);
...
void publish_path(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pubPath)
{
    ...
    // msg_body_pose.header.stamp = nh_ptr->now();
...
int main(int argc, char** argv)
{
    ...
    nh_ptr = rclcpp::Node::make_shared("laserMapping");
    ...
    if (p_pre->lidar_type == AVIA) {
        sub_pcl = nh_ptr->create_subscription<livox_ros_driver2::msg::CustomMsg>(lid_topic, rclcpp::QoS(200000), livox_pcl_cbk);
    } else {
        sub_pcl = nh_ptr->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, rclcpp::QoS(200000), standard_pcl_cbk);
    }
    auto sub_imu = nh_ptr->create_subscription<sensor_msgs::msg::Imu>(imu_topic, rclcpp::QoS(200000), imu_cbk);

    auto pubLaserCloudFullRes = nh_ptr->create_publisher<sensor_msgs::msg::PointCloud2>
            ("/cloud_registered", 1000);
    auto pubLaserCloudFullRes_body = nh_ptr->create_publisher<sensor_msgs::msg::PointCloud2>
            ("/cloud_registered_body", 1000);
    // auto pubLaserCloudEffect  = nh_ptr->create_publisher<sensor_msgs::msg::PointCloud2>
            // ("/cloud_effected", 1000);
    auto pubLaserCloudMap = nh_ptr->create_publisher<sensor_msgs::msg::PointCloud2>
            ("/Laser_map", 1000);
    auto pubOdomAftMapped = nh_ptr->create_publisher<nav_msgs::msg::Odometry> 
            ("/aft_mapped_to_init", 1000);
    auto pubPath          = nh_ptr->create_publisher<nav_msgs::msg::Path> 
            ("/path", 1000);
    // auto plane_pub = nh_ptr->create_publisher<visualization_msgs::msg::Marker>
            // ("/planner_normal", 1000);
//------------------------------------------------------------------------------------------------------
    // 在ROS2中，使用rclcpp的信号处理机制
    ...
    while (rclcpp::ok() && !flg_exit)
    {
        ...
        if(sync_packages(Measures)) 
        {
            if (flg_reset)
            {
                RCLCPP_WARN(nh_ptr->get_logger(), "reset when rosbag play back");
                p_imu->Reset();
            ...
            if (flg_first_scan)
            {
                ...
                if (first_imu_time < 1)
                {
                    first_imu_time = imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9;
                ...
                if(imu_en)
                {
                    ...
                    {
                        while (Measures.lidar_beg_time > imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9) // if it is needed for the new map?
                        {
                            imu_deque.pop_front();
                            if (imu_deque.empty())
                            {
            ...
            {     
                ...
                for (k = 0; k < time_seq.size(); k++)
                {
                    ...
                    {
                        bool last_imu = (imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9) == (imu_deque.front()->header.stamp.sec + imu_deque.front()->header.stamp.nanosec * 1e-9);
                        while (imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9 < time_predict_last_const && !imu_deque.empty())
                        {
                            if (!last_imu)
                            {
                                imu_last = imu_next;
                        ...
                        while (imu_comes) 
                        {
                            ...
                            /*** covariance update ***/
                            double dt = (imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9) - time_predict_last_const;
                            kf_output.predict(dt, Q_output, input_in, true, false);
                            time_predict_last_const = imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9; // big problem
                            
                            {
                                double dt_cov = (imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9) - time_update_last; 

                                if (dt_cov > 0.0)
                                {
                                    time_update_last = imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9;
                    ...
                    {
                        RCLCPP_WARN(nh_ptr->get_logger(), "No point, skip this scan!");
                        idx += time_seq[k];
                        continue;
                    }
                ...
                else
                {
                    ...
                    while ((imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9) > time_current && (((imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9) < Measures.lidar_beg_time + lidar_time_inte )))
                    { // >= ?
                        if (is_first_frame)
                        {
                            {
                                {
                                    while ((imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9) < Measures.lidar_beg_time + lidar_time_inte)
                                    {
                        ...
                        time_current = imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9;
            ...
            else
            {
                ...
                for (k = 0; k < time_seq.size(); k++)
                {
                    ...
                    while (time_current > (imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9)) // && !imu_deque.empty())
                    {
                        imu_deque.pop_front();
                        ...
                        if (dt_cov > 0.0)
                        {
                            ...
                            time_update_last = imu_last.header.stamp.sec + imu_last.header.stamp.nanosec * 1e-9; //time_current;
                        }
                ...
                else
                {
                    ...
                    { // >= ?
                        ...
                        {        
                            // kf_input.predict(dt_cov, Q_input, input_in, false, true);
                            time_update_last = imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9; //time_current;
                        }
                        // kf_input.predict(dt, Q_input, input_in, true, false);

                        t_last = imu_next.header.stamp.sec + imu_next.header.stamp.nanosec * 1e-9;
