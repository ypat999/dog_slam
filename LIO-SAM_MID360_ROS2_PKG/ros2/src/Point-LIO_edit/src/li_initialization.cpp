...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr msg) 
{
    ...
    {
        // ROS2 equivalent of ROS_ERROR
        // We'll handle this with a simple return for now
    ...
    { 
        if (ptr->points.size() > 0)
        {
            ...
            time_buffer.emplace_back(msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9);
        }
    }
    }
...
void imu_cbk(const sensor_msgs::msg::Imu::SharedPtr msg_in) 
{
    ...
    // Convert time stamp
    double adjusted_time = msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9 - timediff_imu_wrt_lidar - time_lag_IMU_wtr_lidar;
    msg->header.stamp = rclcpp::Time(static_cast<uint64_t>(adjusted_time * 1e9));
    ...
    { 
        ...
        if (p_imu->imu_need_init_)
        {
            ...
            while (imu_time < lidar_end_time)
            {
                ...
                imu_time = imu_deque.front()->header.stamp.sec + imu_deque.front()->header.stamp.nanosec * 1e-9; // can be changed
                imu_next = *(imu_deque.front());
            }
        }
        imu_pushed = true;
