...
void Preprocess::process_cut_frame_livox(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg,
    ...
    double last_frame_end_time = (msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9) * 1000;
    uint valid_num = 0;
    uint cut_num = 0;
    ...
    for (uint i = 1; i < valid_pcl_size; i++) {
        ...
        pl_surf[i].curvature += (msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9) * 1000 - last_frame_end_time;
...
Preprocess::process_cut_frame_pcl2(const sensor_msgs::msg::PointCloud2::SharedPtr &msg, deque<PointCloudXYZI::Ptr> &pcl_out,
    ...
    if (lidar_type == VELO16) {
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
            added_pt.curvature = (pl_orig.points[i].timestamp - (msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9)) * 1000;  //s to ms
...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg)
{
  ...
  for (uint i = 0; i < plsize; ++i)
  {
    ...
    double dist = added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z;
    if (dist > (blind * blind) && dist < (det_range * det_range) && !std::isnan(added_pt.x) && !std::isnan(added_pt.y) && !std::isnan(added_pt.z))
    {
