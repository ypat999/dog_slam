...
bool LaserMapping::InitROS(rclcpp::Node::SharedPtr nh) {
    LoadParams(nh);
    SubAndPubToROS(nh);
...
bool LaserMapping::LoadParams(rclcpp::Node::SharedPtr nh) {
    ...
    // Declare and get parameters using ROS2 API
    nh->declare_parameter("path_save_en", true);
    nh->declare_parameter("publish.path_publish_en", true);
    nh->declare_parameter("publish.scan_publish_en", true);
    nh->declare_parameter("publish.dense_publish_en", false);
    nh->declare_parameter("publish.scan_bodyframe_pub_en", true);
    ...
    nh->declare_parameter("common.time_sync_en", false);
    nh->declare_parameter("filter_size_surf", 0.5);
    nh->declare_parameter("filter_size_map", 0.0);
    nh->declare_parameter("cube_side_length", 200.0);
    nh->declare_parameter("mapping.det_range", 300.0f);
    nh->declare_parameter("mapping.gyr_cov", 0.1);
    nh->declare_parameter("mapping.acc_cov", 0.1);
    nh->declare_parameter("mapping.b_gyr_cov", 0.0001);
    nh->declare_parameter("mapping.b_acc_cov", 0.0001);
    nh->declare_parameter("preprocess.blind", 0.01);
    nh->declare_parameter("preprocess.time_scale", 1e-3f);
    nh->declare_parameter("preprocess.lidar_type", 1);
    nh->declare_parameter("preprocess.scan_line", 16);
    nh->declare_parameter("point_filter_num", 2);
    nh->declare_parameter("feature_extract_enable", false);
    nh->declare_parameter("runtime_pos_log_enable", true);
    nh->declare_parameter("mapping.extrinsic_est_en", true);
    nh->declare_parameter("pcd_save.pcd_save_en", false);
    nh->declare_parameter("pcd_save.interval", -1);
    nh->declare_parameter("mapping.extrinsic_T", std::vector<double>());
    nh->declare_parameter("mapping.extrinsic_R", std::vector<double>());
    nh->declare_parameter("ivox_grid_resolution", 0.2f);
    nh->declare_parameter("ivox_nearby_type", 18);

    nh->get_parameter("path_save_en", path_save_en_);
    nh->get_parameter("publish.path_publish_en", path_pub_en_);
    nh->get_parameter("publish.scan_publish_en", scan_pub_en_);
    nh->get_parameter("publish.dense_publish_en", dense_pub_en_);
    nh->get_parameter("publish.scan_bodyframe_pub_en", scan_body_pub_en_);
    ...
    nh->get_parameter("common.time_sync_en", time_sync_en_);
    nh->get_parameter("filter_size_surf", filter_size_surf_min);
    nh->get_parameter("filter_size_map", filter_size_map_min_);
    nh->get_parameter("cube_side_length", cube_len_);
    nh->get_parameter("mapping.det_range", det_range_);
    ...
    nh->get_parameter("preprocess.blind", preprocess_->Blind());
    nh->get_parameter("preprocess.time_scale", preprocess_->TimeScale());
    nh->get_parameter("preprocess.lidar_type", lidar_type);
    nh->get_parameter("preprocess.scan_line", preprocess_->NumScans());
    nh->get_parameter("point_filter_num", preprocess_->PointFilterNum());
    nh->get_parameter("feature_extract_enable", preprocess_->FeatureEnabled());
    nh->get_parameter("runtime_pos_log_enable", runtime_pos_log_);
    nh->get_parameter("mapping.extrinsic_est_en", extrinsic_est_en_);
    nh->get_parameter("pcd_save.pcd_save_en", pcd_save_en_);
    nh->get_parameter("pcd_save.interval", pcd_save_interval_);
    nh->get_parameter("mapping.extrinsic_T", extrinT_);
    nh->get_parameter("mapping.extrinsic_R", extrinR_);
    nh->get_parameter("ivox_grid_resolution", ivox_options_.resolution_);
...
void LaserMapping::SubAndPubToROS(rclcpp::Node::SharedPtr nh) {
    // ROS subscribe initialization
    std::string lidar_topic, imu_topic;
    lidar_topic = nh->declare_parameter<std::string>("common/lid_topic", "/livox/lidar");
    ...
    if (preprocess_->GetLidarType() == LidarType::AVIA) {
        sub_pcl_livox_ = nh->create_subscription<livox_ros_driver2::msg::CustomMsg>(
            lidar_topic, rclcpp::QoS(rclcpp::KeepLast(1)).best_effort(), 
    ...
    sub_imu_ = nh->create_subscription<sensor_msgs::msg::Imu>(
        imu_topic, rclcpp::QoS(rclcpp::KeepLast(100)).best_effort(),
        [this](const sensor_msgs::msg::Imu::SharedPtr msg) { IMUCallBack(msg); });

    // ROS publisher init
    pub_laser_cloud_world_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered", rclcpp::QoS(100000));
    pub_laser_cloud_body_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered_body", rclcpp::QoS(100000));
    pub_laser_cloud_effect_world_ = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered_effect_world", rclcpp::QoS(100000));
    pub_odom_aft_mapped_ = nh->create_publisher<nav_msgs::msg::Odometry>("/Odometry", rclcpp::QoS(100000));
    pub_path_ = nh->create_publisher<nav_msgs::msg::Path>("/path", rclcpp::QoS(100000));
...
void LaserMapping::StandardPCLCallBack(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    Timer::Evaluate(
        [&, this]() {
            scan_count_++;
            double lidar_time = msg->header.stamp.sec + msg->header.stamp.nanosec / 1e9;
            if (lidar_time < last_timestamp_lidar_) {
            ...
            time_buffer_.push_back(lidar_time);
            last_timestamp_lidar_ = lidar_time;
        },
        "Preprocess (Standard)");
    mtx_buffer_.unlock();
}
...
void LaserMapping::IMUCallBack(const sensor_msgs::msg::Imu::SharedPtr msg_in) {
    ...
    if (abs(timediff_lidar_wrt_imu_) > 0.1 && time_sync_en_) {
        double imu_time = msg_in->header.stamp.sec + msg_in->header.stamp.nanosec / 1e9;
        msg->header.stamp = rclcpp::Time(static_cast<int64_t>((timediff_lidar_wrt_imu_ + imu_time) * 1e9));
    }
...
bool LaserMapping::SyncPackages() {
    ...
    double imu_time = imu_buffer_.front()->header.stamp.sec + imu_buffer_.front()->header.stamp.nanosec / 1e9;
    measures_.imu_.clear();
    while ((!imu_buffer_.empty()) && (imu_time < lidar_end_time_)) {
        imu_time = imu_buffer_.front()->header.stamp.sec + imu_buffer_.front()->header.stamp.nanosec / 1e9;
...
void LaserMapping::Savetrajectory(const std::string &traj_file) {
    ...
    for (const auto &p : path_.poses) {
        double stamp = p.header.stamp.sec + p.header.stamp.nanosec / 1e9;
        ofs << std::fixed << std::setprecision(6) << stamp << " " << std::setprecision(15)
