...
# https://blog.csdn.net/lmbcc/article/details/153201018 使用独显，笔记本打开电源高性能
...
def generate_launch_description():
    ...
    controller_manager_node = Node(
        ...
        parameters=[os.path.join(pkg_ros2_livox_simulation, 'config', 'myrobot_ros2_controller.yaml')],
        output='screen'
    )
    ...
    load_joint_state_controller = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['myrobot_joint_state_broadcaster', '--controller-manager', '/controller_manager'],
        output='screen',
    )
    ...
    load_velocity_controller = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['myrobot_velocity_controller', '--controller-manager', '/controller_manager'],
        output='screen',
    )
    # ============================================================================
    ...
    load_controller_event = launch.actions.RegisterEventHandler(
        event_handler=OnProcessExit(
            ...
            on_exit=[load_joint_state_controller, load_velocity_controller]
        )
    )
