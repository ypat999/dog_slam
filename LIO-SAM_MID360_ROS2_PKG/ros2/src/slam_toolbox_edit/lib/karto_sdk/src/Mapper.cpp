  ...
  kt_double smearDeviation, kt_double rangeThreshold)
{
  // invalid parameters
  if (resolution <= 0) {
    std::cerr << "Invalid resolution parameter: " << resolution << std::endl;
    return NULL;
  }
  if (searchSize <= 0) {
    std::cerr << "Invalid searchSize parameter: " << searchSize << std::endl;
    return NULL;
  }
  if (smearDeviation < 0) {
    std::cerr << "Invalid smearDeviation parameter: " << smearDeviation << std::endl;
    return NULL;
  }
  if (rangeThreshold <= 0) {
    std::cerr << "Invalid rangeThreshold parameter: " << rangeThreshold << std::endl;
    return NULL;
  }

  // Check for parameter combinations that could lead to grid index overflow
  kt_double maxSearchSize = 5.0;  // Maximum recommended search size in meters
  if (searchSize > maxSearchSize) {
    std::cerr << "Warning: searchSize (" << searchSize << "m) is larger than recommended maximum (" 
              << maxSearchSize << "m). This may cause grid index overflow." << std::endl;
  }

  kt_double minSmearDeviation = 0.05;  // Minimum recommended smear deviation
  if (smearDeviation < minSmearDeviation) {
    std::cerr << "Warning: smearDeviation (" << smearDeviation << "m) is smaller than recommended minimum (" 
              << minSmearDeviation << "m). This may cause grid index overflow." << std::endl;
  }
  ...
  // Check if search space size is reasonable
  kt_int32u maxSearchSpaceSideSize = 500;  // Maximum recommended grid size
  if (searchSpaceSideSize > maxSearchSpaceSideSize) {
    std::cerr << "Error: Calculated search space size (" << searchSpaceSideSize 
              << ") exceeds maximum recommended size (" << maxSearchSpaceSideSize 
              << "). Please adjust parameters." << std::endl;
    return NULL;
  }
  ...
  // Check if correlation grid size is reasonable
  kt_int32u maxGridSize = 1000;  // Maximum recommended grid size
  if (gridSize > maxGridSize) {
    std::cerr << "Error: Calculated correlation grid size (" << gridSize 
              << ") exceeds maximum recommended size (" << maxGridSize 
              << "). Please adjust parameters." << std::endl;
    return NULL;
  }
  ...
  Matrix3 & rCovariance, kt_bool doPenalize, kt_bool doRefineMatch)
{
  ...
  // Check if the offset is valid to prevent "unable to get pointer in probability search" error
  try {
    // Validate that the offset doesn't cause grid index overflow
    Vector2<kt_double> testPoint(scanPose.GetX(), scanPose.GetY());
    Vector2<kt_int32s> gridCoord = m_pCorrelationGrid->WorldToGrid(testPoint);
    
    // Additional validation for search space probabilities grid
    Vector2<kt_int32s> probGridCoord = m_pSearchSpaceProbs->WorldToGrid(testPoint);
    
    // Check if coordinates are within valid range
    if (!m_pCorrelationGrid->IsValidGridIndex(gridCoord) || 
        !m_pSearchSpaceProbs->IsValidGridIndex(probGridCoord)) {
      std::cerr << "Warning: Scan pose (" << scanPose.GetX() << ", " << scanPose.GetY() 
                << ") results in invalid grid coordinates. Skipping scan matching." << std::endl;
      rMean = scanPose;
      rCovariance(0, 0) = MAX_VARIANCE;
      rCovariance(1, 1) = MAX_VARIANCE;
      rCovariance(2, 2) = 4 * math::Square(m_pMapper->m_pCoarseAngleResolution->GetValue());
      return 0.0;
    }
  } catch (Exception& e) {
    std::cerr << "Error validating scan pose for grid coordinates: " << e.GetErrorMessage() << std::endl;
    rMean = scanPose;
    rCovariance(0, 0) = MAX_VARIANCE;
    rCovariance(1, 1) = MAX_VARIANCE;
    rCovariance(2, 2) = 4 * math::Square(m_pMapper->m_pCoarseAngleResolution->GetValue());
  ...
  kt_bool doPenalize, Pose2 & rMean, Matrix3 & rCovariance, kt_bool doingFineMatch)
{
  ...
  for (kt_int32u i = 0; i < poseResponseSize; i++) {
    ...
    if (!doingFineMatch) {
      ...
      try {
        // Check if grid coordinates are valid before getting data pointer
        if (!m_pSearchSpaceProbs->IsValidGridIndex(grid)) {
          std::cerr << "Warning: Grid coordinates (" << grid.GetX() << ", " << grid.GetY() 
                    << ") out of range for search space probabilities grid ("
                    << m_pSearchSpaceProbs->GetWidth() << "x" << m_pSearchSpaceProbs->GetHeight() 
                    << "). Skipping this pose." << std::endl;
          continue;  // Skip this pose instead of throwing an error
        }
        
        ptr = (kt_double *)(m_pSearchSpaceProbs->GetDataPointer(grid));  // NOLINT
      } catch (Exception& e) {
        std::cerr << "Exception in probability search at grid coordinates (" 
                  << grid.GetX() << ", " << grid.GetY() << "): " << e.GetErrorMessage() << std::endl;
        std::cerr << "Grid size: " << m_pSearchSpaceProbs->GetWidth() << "x" 
                  << m_pSearchSpaceProbs->GetHeight() << std::endl;
        std::cerr << "Search space offset: " << rSearchSpaceOffset.GetX() << ", " 
                  << rSearchSpaceOffset.GetY() << std::endl;
        std::cerr << "Search space resolution: " << rSearchSpaceResolution.GetX() << ", " 
                  << rSearchSpaceResolution.GetY() << std::endl;
        continue;  // Skip this pose instead of throwing an error
      } catch (...) {
        std::cerr << "Unknown exception in probability search at grid coordinates (" 
                  << grid.GetX() << ", " << grid.GetY() << ")" << std::endl;
        continue;  // Skip this pose instead of throwing an error
      }

      if (ptr == NULL) {
        std::cerr << "Warning: NULL pointer returned from GetDataPointer at grid coordinates (" 
                  << grid.GetX() << ", " << grid.GetY() << "). Skipping this pose." << std::endl;
        continue;  // Skip this pose instead of throwing an error
      }
