...
class BufferedImuFilter(Node):
    def __init__(self):
        ...
        self.jump_threshold = 50.0 # 跳变检测阈值（标准差的倍数）
    ...
    def detect_and_handle_jump(self, new_value, buffer, axis_name, data_type):
        ...
        if mean != 0:
            jump_ratio = abs(new_value - mean) / mean
            if jump_ratio > self.jump_threshold:
                self.get_logger().warning(f'检测到{data_type} {axis_name}轴跳变: {new_value:.3f} -> {mean:.3f} (ratio: {jump_ratio:.2f})')
                return mean
        return new_value
